...
const app = express();
app.set('trust proxy', 1);require('dotenv').config();
const express = require('express');
const mongoose = require('mongoose');
const session = require('express-session');
const MongoStore = require('connect-mongo').default;
...
app.get('/payment-instruction/:id', isLogin, async (req, res) => {
  try {
    ...
    if (!order || order.user_id.toString() !== req.session.user._id.toString()) {
...
app.post('/orders/confirm-payment/:id', isLogin, upload.single('payment_proof'), async (req, res) => { 
  try { 
    ...
    await Order.findByIdAndUpdate(req.params.id, { status: 'Menunggu Konfirmasi', payment_proof: req.file.path }); 
...
app.post('/products/add', isLogin, isAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    const image = req.file ? req.file.path : 'https://via.placeholder.com/300x300.png?text=No+Image';
...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!user.isVerified) {
      req.flash('error', 'Akun belum diverifikasi. Silakan daftar ulang untuk mendapatkan kode OTP baru.');
      return res.redirect('/login');
    }
...
app.post('/register', async (req, res) => {
  try {
    ...
    // Jika ada akun dengan username/email ini yang belum verifikasi, hapus agar bisa dipakai ulang
...
app.get('/delete-account', isLogin, async (req, res) => {
  try {
    ...
    // Hapus data customer terkait jika ada
    await Customer.deleteOne({ user_id: userId });
    // Hapus akun user dari database
    await User.findByIdAndDelete(userId);
    
    // Hancurkan session (logout otomatis)
    req.session.destroy((err) => {
      if (err) {
        console.log(err);
        return res.send('Gagal menghapus akun.');
      }
      res.redirect('/'); // Kembali ke halaman utama
    });
  } catch (err) {
    console.log(err);
    res.send('Gagal menghapus akun.');
  }
});
